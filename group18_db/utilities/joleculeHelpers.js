...
const joleculeHelpers = function (pdb, energyCutoffSet) {
  ...
  const baseLocalPath = function () { return `${config.web.baseStatic}/data/${pdb}` }

  const mapFileRemotePath = function (nobleGas) { return `${MAP_FILE_PATH}/${pdb}/${pdb}.${nobleGas}.map` }
  const mapSharedPath = function () { if (MAP_SHARED_FILE_PATH) { return `${MAP_SHARED_FILE_PATH}/${pdb}` } else { return false } }
  const mapFileSharedPath = function (nobleGas) { if (MAP_SHARED_FILE_PATH) { return `${MAP_SHARED_FILE_PATH}/${pdb}/${pdb}.${nobleGas}.map` } else { return false } }
  const mapLocalPath = function (nobleGas) { return `${config.web.baseStatic}/data/${pdb}/maps/${nobleGas}` }
  const mapFileLocalPath = function (nobleGas) { return `${config.web.baseStatic}/data/${pdb}/maps/${nobleGas}/${pdb}.${nobleGas}.map` }

  const pdbFileRemotePath = function () {
    const pdbGroup = pdb.substring(1, 3)
    return `${PDB_FILE_PATH}/${pdbGroup}/pdb${pdb}.ent.gz`
  }
  const pdbStructureFileLocalPath = function () { return `${config.web.baseStatic}/data/${pdb}/pdbs/pdb${pdb}.ent.gz` }
  const pdbFileLocalPath = function () { return `${config.web.baseStatic}/data/${pdb}/pdbs/${pdb}.pdb` }

  const processedPdbLocalPath = function () { return `${config.web.baseStatic}/data/${pdb}/pdbs/${energyCutoffSet}` }
  const processedPdbFileLocalPath = function (nobleGas) { return `${config.web.baseStatic}/data/${pdb}/pdbs/${energyCutoffSet}/${pdb}.${nobleGas}.pdb` }

  const dataServerLocalPathClient = function () { return `/data/${pdb}/dataServers/${energyCutoffSet}` }
  const dataServerLocalPath = function () { return `${config.web.baseStatic}/data/${pdb}/dataServers/${energyCutoffSet}` }
  const dataServerFileLocalPath = function (i) { return `${config.web.baseStatic}/data/${pdb}/dataServers/${energyCutoffSet}/data-server${i}.js` }
  const dataServerRoute = function () { return `/data/${pdb}/${energyCutoffSet}` }
  ...
  const getEnergyCutOffs = function () {
    if (exports.isDefaultEnergyCutoffSet) {
      ...
      return [energyCutoffSet, energyCutoffSet, energyCutoffSet, energyCutoffSet, energyCutoffSet]
    } else {
      throw new Error("'" + energyCutoffSet + "' is not a valid energyCutoffSet (value must be between " + MAX_ENERGY_CUTOFF + ' and ' + MIN_ENERGY_CUTOFF + '). ')
    }
  }
  ...
  const getPdbFile = async function () {
    ...
    try {
      ...
      throw new Error('Failed to find ' + pdb + ' PDB File due to the following error: ' + err + " click <a href='/flushcache/" + pdb + '/' + energyCutoffSet + "'>here to retry</a>")
    }
  }
  ...
  const ensureFileWithPreProcessingScript = async function (localFilePath, args) {
    if (fs.existsSync(localFilePath)) {
      ...
      if (fs.existsSync(localFilePath)) {
        ...
        throw new Error('failed to create ' + localFilePath + ' with cutoff of ' + args[1])
      }
    }
  }
  ...
  const ensurePreProcessingFiles = function () {
    ...
    return Promise
            ...
            .catch(function (err) { throw new Error('Failed to PreProcess map files due to the following error: ' + err + " click <a href='/flushcache/" + pdb + '/' + energyCutoffSet + "'>here to retry</a>") })
  }
  ...
  const runJoleculePreProcessing = function (nobleGas, energyCutoff) {
    ...
    return runScriptAsync(PREPROCESSING_SCRIPT, ['-e', nobleGas, '-u', energyCutoff, '-s', SPACIAL_CUTOFF, '-o', processedPdbLocalPath() + '/', pdb], { cwd: mapPath + '/' })
  }
  ...
  const ensureJoleculeStatic = async function () {
    ...
    if (checkJoleculeStaticFiles()) {
      ...
      try {
        ...
        throw new Error('Failed to Build Jolecule Data_Server due to the following error: ' + err + " click <a href='/flushcache/" + pdb + '/' + energyCutoffSet + "'>here to retry</a>")
      }
    }
  }
